...
need, and bound it: `utterance_limit` defaults to 200 and a 200-row page of a busy
sitting is over this gateway's response cap, so the call is refused and you get
nothing at all. Pass `utterance_limit=25` and walk `next_utterance_offset` only if
you must. Requesting the transcript across a day's events to find one clip is how a
morning note turns into a megabyte. The official record has no timestamps, so
